...
`DeviceRefreshSnapshotReducer` 将设备刷新快照与当前会话状态归约为单一应用计划，联合决定连接阶段、安装身份、有效期、缓存失效、通用关键动作抑制和自动续期授权结论；模块不调度任务、不发送通知，也不直接修改界面。`AutomaticRefreshAuthorizer` 把自动续期评估结论收敛为允许、延后或阻止及其类型化原因：完整且一致的同轮证据可以直接进入自动续期评估；来源降级时，只有固定设备稳定 ID 精确匹配、并在同一轮重新核验目标 Bundle 已安装且有效期可信，才允许启动倒计时和部署预检。来源冲突、只读策略、缓存动作证据、仅观察刷新或安装证据不可用仍然失败关闭。

自动续期评估授权不构造 `DeploymentTarget`，也不授权真正的安装提交。倒计时结束后仍由 `DeploymentPreflightWorkflow` 重新核对设备、锁态、环境、App 目标、Xcode destination 和安装身份，最终设备复核仍必须通过 `DeploymentTargetAuthorizer` 的完整且一致证据要求。授权、延后和阻止原因作为自动化诊断事件持久化并投影到诊断界面，便于区分“尚未进入预检”和“预检后等待或失败”。

`RefreshStateSettlement` 集中处理设备观察、安装身份和部署结果到 `AppState` 的字段映射。调用方通过类型化事件描述部署启动、进程归属、观察结果、自动续期授权、提醒送达和证据失效，模块以“先复制、再原子保存、最后发布”的顺序提交状态。持久化失败时保留旧状态并进入可见阻断；新增跨字段不变量应优先落在这些归约模块，而不是继续散落到 ViewModel 分支。
...
flowchart TD
    ...
    B -- "是" --> AUTH{"自动续期评估是否获准？"}
    AUTH -- "冲突 / 缓存 / 证据不可用" --> Z
    AUTH -- "完整证据，或固定设备与 App 同轮精确核验" --> C["5 秒可取消倒计时"]
    C --> D["重新扫描并按完整设备 ID 执行部署预检"]
...
`StatusBarController` 负责 AppKit 生命周期、菜单栏状态项、原生右键菜单和主窗口显示隐藏。`MainPanelView` 使用顶部命令栏、双列状态工作台及最近历史条；设置与完整历史覆盖内容区，关闭时返回工作台，设置草稿仍由原有 ViewModel 持有。设置页按`目标 / 续期 / 局域网 / 通用`四个分类组织内容。前三类配置草稿通过固定保存栏统一提交；`通用`中的登录启动直接同步系统服务，诊断动作也不进入草稿。`InterfaceStyle` 通过应用偏好独立保存，使用环境值向卡片和控件传播原生/玻璃风格，不参与部署配置、草稿提交或服务启停。`InterfaceSurface` 集中处理材质、圆角与减少透明度，`RecentHistoryStrip` 只消费现有历史分页结果。具体视觉和交互契约见 UI 规范。
