...
- `project-drafts/`：本地开发方案、设计稿及配套素材工作区；除用于保留目录的 `.gitkeep` 外，其余内容默认由 Git 忽略。每项工作可在其中按主题新增子目录和文件。
...
- AI 生成的开发方案、设计稿和配套素材应保存到 `project-drafts/` 下按主题划分的子目录中，不直接新增到 `docs/`；该工作区内容仅用于讨论和迭代，不作为项目事实源，也不得被强制加入 Git。
...
- 主界面左下角只常驻显示 Marketing Version；Build Version 只在悬停说明、辅助功能文本和诊断信息中展示。发布版本只修改 `config/release-metadata.json`，不得在界面代码、README 或测试中复制当前版本值。
...
- `RenewalIconPresentation` / `SidebarBrandIcon`：将续期图标的业务颜色与运动阶段分离；绿色只表示当前成功反馈，历史成功不得恢复绿色；侧栏图标使用 `64pt` Renewal Loop、支持 Reduce Motion，并在主面板不可见时暂停且恢复后不追赶后台时间。
...
- “当前状态”主旅程使用统一活动反馈卡，承载检查、自动刷新倒计时、部署、自动恢复、结算结果和通用反馈；部署期间在任务卡下方常驻有界实时输出，并可通过主面板浮层查看完整日志。
